...
## Company(회사) API 모듈 주요 구현 내용

- **회사 등록 (Create Company)**
  - 관리자 권한에서만 사용 가능하며, 회사명/회사코드를 입력받아 신규 회사 데이터를 생성합니다.

- **회사 목록 조회 및 검색 Get Companies**
  - 관리자 권한만 접근 가능
  - 검색 조건(회사명/코드) 및 페이지네이션 지원
  - 전체 회사목록, 총 개수, 페이지 정보 반환

- **회사별 소속 유저 목록 조회 Get Company Users**
  - 관리자 권한에서 회사별 직원(유저) 목록 반환
  - 이름, 이메일, 사번, 전화번호 등 기본 정보 포함
  - 검색/페이지네이션 동시 지원

- **회사 정보 수정 Update Company**
  - 관리자가 회사명/회사코드 등 정보 수정
  - 잘못된 ID나 없는 회사 처리 시 404 오류 메시지 반환

- **회사 삭제 Delete Company**
  - 관리자가 특정 회사 데이터를 삭제
  - 성공 시 메시지, 실패 시 사유/오류 메시지 반환

모든 API는 JWT 인증(Authorization 헤더) 및 관리자권한이 필요 
일관된 에러 핸들링을 적용하여 프론트엔드와 연동이 용이하도록 구성
...
- **계약 관리 CRUD API**
- 차량, 고객 정보를 불러와 계약 등록 기능
- 유저의 회사에 등록된 계약의 수정, 삭제 기능(계약 생성자만 가능)
- 계약 거래 상태(차량 확인, 가격 협의, 계약 성공, 계약 실패)에 따른 드래그 앤 드랍 기능을 위한 모든 계약 불러오기
- 고객, 담당자명으로 검색 기능
- 계약 생성과 계약 성공에 따른 차의 상태, 고객의 계약 수 변동 기능
- **DB 스키마 설계**
  - 팀원들과 공동 작업
  - 계약 생성을 위한 다른 유저 고객 등 다양한 테이블과의 구조 설계
- **에러의 최소화를 위한 디버깅**
- 계약의 상태가 성공, 실패에서 이전 단계로 이동 불가 기능
...
<br> <br/>
- **고객 관리 API**
  - 고객 목록 조회(이메일, 이름으로 검색 가능), 고객 단일 조회 구현 
  - 고객 정보 수정, 고객 삭제, 고객 추가 구현 (고객과 직원이 같은 회사일 때만 가능)
  - 파라미터들에 대한 유효성 검사 
  
- **DB 스키마 설계**
  - 프로젝트 시작 전, 철저한 API 설계를 통한 작업 소요 감소 
  - 데이터 정규화 및 API 구현, 웹사이트의 목적에 맞는 스키마 설계
 
- **대시보드 API**
  - 한달 사이 매출, 매출 증가율, 완료된 계약과 진행중인 계약 수, 차종별 계약 수와 이익을 구현했습니다
  - 값들마다 서비스 계층에서 각각 처리하여, 책임을 분리했습니다. 
  - 웹사이트 설계에 적절하도록 값들을 검토하였습니다. 
    
- **함수를 이용한 추상화**
  - 프론트엔드에서 온 데이터를 백엔드 데이터에 맞게 변환하는 함수를 적용하여 추상화
  - 프론트에서 요구하는 포맷으로 변환하는 포매팅 함수
  - 유저와 유저의 회사에 대한 유효성 검사, 인가를 함수로 구현 
  - 더 읽기 쉽고 유지 보수가 원활한 코드
