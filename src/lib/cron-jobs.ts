...
// ------------------------------------------------------------------
const cloudinaryDeleted = async (publicIdsToDelete: string[], resourceType: string) => {
  try {
...
const findAndDeleteOrphanDocuments = async () => {
  ...
  try {
    ...
    const publicIdsToDelete = orphanDocuments.map((resource) => resource.publicId);
    const documentIdsToDelete = orphanDocuments.map((resource) => resource.id);

    console.log(
      `[CRON] 삭제 대상 ${logName} ${documentIdsToDelete.length}개 발견. Public IDs: ${publicIdsToDelete.join(', ')}`,
    );

    // Cloudinary에서 파일 삭제
    const isCloudinaryDeleted = await cloudinaryDeleted(publicIdsToDelete, resourceType);
    ...
    console.log(`[DB] 총 ${contractDocument.count}개의 미연결 ${logName}를 삭제했습니다.`);
...
// 미연결된 이미지 삭제
// ------------------------------------------------------------------
const findAndDeleteOrphanImages = async () => {
  const logName = '이미지';
  ...
  // 조건: contractId가 null이고, 생성된 지 24시간(1일)이 지난 문서 조회
  try {
    const orphanDocuments = await prisma.image.findMany({
      where: {
        user: null,
        createdAt: {
          lt: oneDaysAgo,
    ...
    // DB에서 일괄 삭제 (Cloudinary 삭제 성공시에만 실행)
    const contractDocument = await prisma.image.deleteMany({
      where: {
        id: {
          in: documentIdsToDelete,
        },
...
export const startCleanupJob = () => {
  ...
  cron.schedule(
    '0 4 * * *',
    async () => {
      await findAndDeleteOrphanImages();
    },
    {
      timezone: 'Asia/Seoul',
    },
