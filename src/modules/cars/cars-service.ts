...
export class CarService {
    ...
    public createCar = async (
        ...
        if (!modelId && carData.manufacturer && carData.model) {
            const foundModel = await this.carRepository.findModelByManufacturerAndName(
            ...
            if (!foundModel) {
                throw new Error(
                    `제조사 "${carData.manufacturer}", 모델 "${carData.model}"을 찾을 수 없습니다.`
                );
            }
    ...
    public bulkCreateCars = async (carsData: Array<{
        modelId: number;
        companyId: number;
        carNumber: string;
